...
'''
start = time.time()
dg = DirectoryDataGenerator(base_directories=["/mnt/DataStore/projects_share/datasets/Dogs/test"], augmentor=True, preprocessors=preprocess_input, batch_size=1, target_sizes=(224,224), shuffle=False)

end = time.time()
total = end - start
print("init time: ", total)
...
for i in range(10):
    ...
    for batch in X:
        for img in batch:
            cv2.imshow("", img)
            cv2.waitKey(1)
            cv2.imwrite("BGR_preprocessing.jpg", img)
            import sys
            sys.exit(0)
    end = time.time()
    total = end - start
    print(i, "batch time: ", total)
'''