...
gpu_options = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction = 0.28)
...
print('&&&&&&&&&&&&&&&&&& Wandb_log condition &&&&&&&&&&&&&&&&& -->', wandb_log)

# ============== Wandb ======================= #
inp_size = image_size + [3]
if wandb_log:
...
model = construct_model(
    ...
    freeze_backbone = freeze_backbone,  # Need to verify this with different model backbone output at different iteration
    gnn1_layr = gnn1_layr,
    gnn2_layr = gnn2_layr,
    # summary = summary
    )
...
if calflops:
    r = get_flops( model, tf.compat.v1.placeholder('float32', shape=(1, inp_size[0], inp_size[1], inp_size[2])) ) 
