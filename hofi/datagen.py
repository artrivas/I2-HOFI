...
class DirectoryDataGenerator(tf.keras.utils.Sequence):
    ...
    def center_crop(self, img, target_size):
        ...
        # Calculate coordinates to crop the image around the center
    ...
    def cv2_image_augmentation(self, img, theta=20, tx=10, ty=10, scale=1., flip=False):   # origial one
        
        if scale != 1.:
            scale = np.random.uniform(1-scale, 1+scale)
            # print(' ~~~~~~~~ INTO random Scaling ~~~~~~~~~~~ ', scale)
        ...
        # print(' ~~~~~~~~ AFTER WRAPAFFINE ~~~~~~~~~~~ ', np.shape(image))
        if flip:            
