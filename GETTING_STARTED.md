...
Refer to the [tf.keras.applications](https://www.tensorflow.org/api_docs/python/tf/keras/applications) page for a complete list of available models. Note that the I2-HOFI model requires backbone models with channel dimensions in multiples of 2. For models with different channel dimensions, add a convolutional layer to adjust them to the nearest multiple of 2. Additionally, optimal configuration settings may vary between different backbone models compared to Xception.
